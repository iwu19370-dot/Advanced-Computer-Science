...
public interface ElementalAttacks {
    ...
    default void attack(Monster opponent) {
        ...
        if (chances < 45) {
            System.out.println(thisMonster().attackNameOne);
            performSingleElementalAttack(1, opponent);
        } else if (chances < 90) {
            System.out.println(thisMonster().attackNameTwo);
            performSingleElementalAttack(2, opponent);
        } else {
            System.out.println(thisMonster().attackNameOne);
            System.out.println(thisMonster().attackNameTwo);
    ...
    default void performSingleElementalAttack(int attackNumber, Monster opponent) {
        ...
        int damage = 0;
        int baseDamage = 10;
        double attack = thisMonster().getAttack();
        double opponentDefense = thisMonster().getDefense();
        double criticalMultiplier = 0.0;
        if (didAttackCrit(thisMonster().getSpeed()) == true) {
            criticalMultiplier = 2.0;
        } else {
            criticalMultiplier = 1.0;
        }
        double stab = 0.0;
        if (thisMonster().getElement() == opponent.getElement()) {
            stab = 1.5;
        } else {
            stab = 1;
        }
        double effectivenessMultiplier =
                getElementalMultiplier(thisMonster().getElement(), opponent.getElement());
        damage = (int) (baseDamage * (attack / opponentDefense) * stab * effectivenessMultiplier
                * criticalMultiplier);
        opponent.takeDamage(damage);
        if (isSuperEffectiveAgainst(thisMonster().getElement(), opponent.getElement()) == true) {
            System.out
                    .println(thisMonster().getName() + " used an attack! It was super effective!");
        }
        if (isSuperEffectiveAgainst(thisMonster().getElement(), opponent.getElement()) == false) {
            System.out.println(
                    thisMonster().getName() + " used an attack! It was not very effective...");
        }
    }
